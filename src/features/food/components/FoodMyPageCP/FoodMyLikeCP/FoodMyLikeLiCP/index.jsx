...
const FoodMyLikeLiCP = ({ ftId, onDeleteLike, onDeleteSms, onAddSms }) => {
  ...
  const getBusinessStatus = (ft) => {
    ...
    if (currentMinutes < startMinutes) {
      ...
      return { status: "영업종료", color: "var(--food-red)" };
    }
  };
            ...
            <p className="flexBetween">
              ...
              <span style={{ fontSize: "1rem" }}>
                <span style={{ cursor: "pointer" }} onClick={() => onDeleteLike(ftData.truckId)}>
                  <FontAwesomeIcon icon={faHeart} style={{ color: "var(--food-red)", paddingRight: "0.5rem" }} />
                </span>
            ...
            <ul className="schedule">
              {ftData.schedule?.slice().map((schedule, idx) => (
                <li
                  key={idx}
                  style={{
                    color: !schedule.holiday ? "var(--food-red)" : idx === today ? "var(--green-food-accent)" : "",
                  }}>
                  <span>
