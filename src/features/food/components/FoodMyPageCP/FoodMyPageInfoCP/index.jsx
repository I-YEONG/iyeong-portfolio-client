  ...
  return (
    <section css={foodMyPageInfoCPStyle(isPc)}>
      ...
      <div>
        ...
        <FoodInputCP title={"e-mail"} value={userData.email} lock={true} />
        <div>
          <FoodInputCP title={"닉네임"} onChangeHandler={onChangeNickName} value={nickName} />
          ...
          {userData.nickname !== nickName && (
            <div style={{ marginTop: "4px" }} onClick={onUpdateNickName}>
              <FoodOutLineButtonCP color="#A47764">닉네임 변경 </FoodOutLineButtonCP>
            </div>
          )}
        </div>
        <FoodInputCP title={"역할"} value={userData.role} lock={true} />

        <div className="col">
          <FoodInputCP title="전화번호" value={phone} lock={true} ex="숫자만 입력하세요" />
        </div>

        <div>
          <FoodInputCP title="비밀번호" value={password} onChangeHandler={onChangePassword} pw={true} ex="8자 이상 20자 이하" />
          <span className="error" style={{ visibility: pwError ? "visible" : "hidden" }}>
            비밀번호는 8자 이상 20자 이하로 입력해야 합니다.
          </span>

          <FoodInputCP title="비밀번호 확인" value={confirmPassword} onChangeHandler={onChangeConfirmPassword} pw={true} />
          <span className="error" style={{ visibility: pwConfirmError ? "visible" : "hidden" }}>
            비밀번호가 일치하지 않습니다.
          </span>
          {password && confirmPassword && (
            <div style={{ marginTop: "4px" }} onClick={onUpdatePassword}>
              <FoodOutLineButtonCP color="#A47764">비밀번호 변경</FoodOutLineButtonCP>
            </div>
          )}
        </div>
