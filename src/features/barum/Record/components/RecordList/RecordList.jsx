...
const RecordList = ({ listData = [], isLoading = false, isError = false, skeletonCount = 2 }) => {
  ...
  return (
    <section css={{ ...theme.barum.flex.colStart, gap: "16px", height: "100%" }}>
      {/* 1. 로딩 중일 때 스켈레톤 표시 */}
      {isLoading && Array.from({ length: skeletonCount }).map((_, idx) => <BarumRecordItem key={`record-skeleton-${idx}`} isSkeleton />)}
      ...
      {/* 3. 로딩이 끝나고 데이터가 있을 때 */}
      {!isLoading && hasData && listData.map((item, idx) => <BarumRecordItem key={`${item?.date || "record"}-${idx}`} data={item} />)}
    </section>
  );
};
