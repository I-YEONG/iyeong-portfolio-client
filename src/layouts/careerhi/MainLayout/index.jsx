...
const MainContentLayout = ({ children, page = "main", fixed = false, scroll = false }) => {
  ...
  return (
    <section
      className={`z-40 h-[calc(100%-5.125rem)] min-h-[calc(100%-5.125rem)] w-full bg-gray-100 flex items-stretch select-none overflow-hidden relative ${
        scroll ? "overflow-y-auto" : ""
      }`}>
      {/* 왼쪽 영역 (남는 공간의 절반) */}
      <aside className={`z-45 flex-1 box-border px-[2vw] pt-14 h-[calc(100%-5.125rem)] ${fixed ? "fixed top-20.5 w-[calc(50vw/2)]" : "relative"}`}>
        <div className="flex flex-col gap-1.5">
          ...
          <div
            onClick={() => nav("/project/careerhi/roadmap/list")}
            className={`isMobileHidden flex items-center transition-colors duration-200 w-full max-w-[16vw] cursor-pointer select-none hover:bg-white h-14 rounded-2xl px-[10%] B2_bold ${
