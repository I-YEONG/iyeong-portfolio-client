...
// style.js 에서 분리한 Emotion 스타일들 임포트
import {
  foodFaqPageMainStyle,
  foodFaqHeaderStyle,
  foodQnaSectionStyle,
  foodContactSectionStyle,
  foodContactCardStyle,
  foodContactInfoItemStyle,
  foodQuickLinkSectionStyle,
} from "./style";
import { FoodMainLayOut } from "@/layouts";
import { FoodButtonCP, FoodOutLineButtonCP, FoodSelectInputCP, FoodSimpleAccordionCP, FoodTextAreaInputCP } from "@/features/food/components";
...
const faqData = {
    ...
    items: [
      {
        ...
        answer:
          "길맛로드는 길거리 포장마차와 푸드트럭의 실시간 위치 정보를 제공하는 플랫폼입니다. 고객은 주변 푸드트럭을 쉽게 찾을 수 있고, 사장님들은 위치 정보를 등록하여 고객과 소통할 수 있습니다.",
      },
      {
        id: "g2",
  ...
  user: {
    ...
    items: [
      {
        id: "u1",
        question: "푸드트럭의 위치 정보가 정확하지 않아요.",
        answer: '위치 정보는 푸드트럭 사장님이 직접 업데이트합니다. 정확하지 않은 정보를 발견하시면 "제보하기" 기능을 통해 제보해주세요.',
      },
      {
        id: "u2",
...
const FoodFaqPage = () => {
  ...
  return (
    <FoodMainLayOut>
      <div css={foodFaqPageMainStyle(isPc)}>
        <section css={foodFaqHeaderStyle(isPc)}>
          <h1>궁금한 것이 있으신가요?</h1>
          <p>자주 묻는 질문들을 확인해보세요. 원하는 답변을 찾지 못하셨다면 문의해주세요.</p>
        </section>

        {Object.entries(faqData).map(([key, section]) => (
          <section key={key} css={foodQnaSectionStyle(isPc)}>
            <div>
              <h2>{section.title}</h2>
              <FoodSimpleAccordionCP items={section.items} />
            </div>
          </section>
        ))}

        <section id="qna" css={foodContactSectionStyle(isPc)}>
          {/* Contact Form Card */}
          <div css={foodContactCardStyle(isPc)}>
            <div>
            ...
            <div className="col">
              <FoodSelectInputCP title="문의 유형" essential="true" listData={askCategoryList} onChangeHandler={onChangeAskCategory} />
              {askCategoryError && <span className="error">문의 유형을 선택하세요.</span>}
            </div>
            ...
            <div className="col">
              <FoodTextAreaInputCP
                ...
                minRows={1}
              />
              {askTitleError && <span className="error">제목은 2자 이상 20자 이하로 입력해야 합니다.</span>}
            </div>
            ...
            <div className="col">
              <FoodTextAreaInputCP
                ...
                minRows={5}
              />
              {askContentError && <span className="error">문의 내용을 작성해주세요.</span>}
            </div>
            ...
            <div className="col">
              <FoodTextAreaInputCP
                ...
                minRows={1}
              />
              {askContactError && <span className="error">이메일 형식 또는 전화번호 형식이 올바르지 않습니다.</span>}
            </div>

            <FoodButtonCP onClick={validateInquiryForm}>문의하기</FoodButtonCP>
          </div>
          ...
          <div css={foodContactCardStyle(isPc)}>
            ...
            <div css={foodContactInfoItemStyle(isPc)}>
            ...
            <div css={foodQuickLinkSectionStyle(isPc)}>
              <h2>자주 이용하는 기능</h2>
              <FoodOutLineButtonCP color="black">
