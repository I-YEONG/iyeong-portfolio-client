...
import { routineSelfieAtom } from "@/atoms/selfieAtom";
...
import { useSaveRoutineRecord } from "@/features/barum/Routine/hooks/useSaveRoutineRecord";
...
const RoutineResult = () => {
  ...
  const style = { ...theme.barum.flex.colStart, alignItems: "start", gap: "20px", width: "100%" };
  ...
  return (
    <BarumHomeLayout
      buttons={
        <BarumBarButton colorTheme={isSaving ? "none" : "green"} clickFun={isSaving ? () => {} : handleSaveRecord}>
          {isSaving ? "저장 중..." : "기록 저장"}
        </BarumBarButton>
      }>
      <p css={{ ...theme.barum.fonts.caption, color: theme.barum.colors.ink3 }}>{today}</p>
      <p css={{ ...theme.barum.fonts.display, color: theme.barum.colors.ink1, marginBottom: "20px" }}>오늘의 루틴</p>
      ...
      <section css={style}>
        ...
        {/* 날씨 */}
        <BarumResultWeatherBox isLoading={!resultData} data={resultData?.weather || {}} />

        {/* 충돌 crash */}
        <BarumCrashBox isLoading={!resultData} data={resultData?.conflicts || []} />

        {/* 바를 것 */}
        <BarumApplyBox isLoading={!resultData} data={resultData?.routine?.apply || []} />

        {/* 뺄 것 */}
        <BarumSkipBox isLoading={!resultData} data={resultData?.routine?.skip || []} />
      </section>
    </BarumHomeLayout>
  );
};
