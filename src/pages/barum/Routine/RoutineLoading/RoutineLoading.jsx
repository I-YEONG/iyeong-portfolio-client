import { useRoutineStream } from "@/features/barum/Routine/hooks/useRoutineStream";
...
const RoutineLoading = () => {
  ...
  useEffect(() => {
    ...
    switch (errorCode) {
      case "EMPTY_VANITY":
        ...
        navigate("/project/barum/routine/create/error?type=EMPTY_VANITY");
        break;

      case "AI_TIMEOUT":
        // 25초 내 루틴 생성 실패
        console.error("AI 응답 초과:", error.message);
        navigate("/project/barum/routine/create/error");
        break;

      case "EXTERNAL_API_ERROR":
        // 그 외 AI 서버 오류
        console.error("외부 API 오류:", error.message);
        navigate("/project/barum/routine/create/error?type=EXTERNAL_API_ERROR");
        break;
  ...
  useEffect(() => {
    ...
    if (progress === 100) {
      const timer = setTimeout(() => {
        ...
        navigate("/project/barum/routine/result", { state: { savedData, resultData } });
      }, 2000);
  ...
  return (
    <BarumBasicLayout styleObj={routineLoadingLayoutStyle}>
